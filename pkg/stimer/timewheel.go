/*
 * @Author: bsun
 * @Date: 2024-07-01 14:02:19
 * @Last Modified by: bsun
 * @Last Modified time: 2024-07-01 14:03:33
 */
package stimer
...
func NewTimeWheel(name string, interval int64, scales int, maxCap int) *TimeWheel {
	...
	zaplog.LoggerSugar.Infof("Init timerWhell name = %s is Done!", tw.name)
	return tw
}
...
func (tw *TimeWheel) addTimer(tID uint32, t *Timer, forceNext bool) error {
	defer func() error {
		if err := recover(); err != nil {
			...
			zaplog.LoggerSugar.Errorf("addTimer function err : %s", err)
...
func (tw *TimeWheel) AddTimeWheel(next *TimeWheel) {
	...
	zaplog.LoggerSugar.Infof("Add timerWhell[%s]'s next [%s] is succ!", tw.name, next.name)
}
...
func (tw *TimeWheel) Run() {
	go tw.run()
	zaplog.LoggerSugar.Infof("timerwheel name = %s is running...", tw.name)
}
